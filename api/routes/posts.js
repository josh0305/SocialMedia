...
router.put("/:id", async(req, res)=>{
    try{
        ...
        if(post.userId === req.body.userId){
            ...
            res.status(200).json("You have updated the post")
        }else{
...
router.put("/:id/like", async(req, res)=>{
    try{
        ...
        if(!post.likes.includes(req.body.userId)){
            ...
            res.status(200).json("You have desliked this post")
        }
    }catch(err){
        res.status(500).json(err)
    }
...
router.get("/timeline/:userId", async(req, res)=>{
    try{
        ...
        const friedsPosts = await Promise.all(
            curUser.followings.map((friendId) => {
                return Post.find({ userId: friendId })
            })
        )
        res.status(200).json(userPosts.concat(...friedsPosts))
    }catch(err){
        res.status(500).json(err)
    }
})
